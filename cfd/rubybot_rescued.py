...
def on_ready(): 
	...
	global taboo_teams
	taboo_teams = []
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='246194661763317761')) #red team
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='246194717946019840')) #blue team
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='246194772253999104')) #wildcats
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='246194825370533889')) #the real illuminati
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='246194907302199296')) #cool and new team
	taboo_teams.append(discord.utils.get(taboo_server.roles, id='254164527271247872')) #number one
	...
	loop.create_task(background_check_feed(loop, 'http://loreweaver-universe.tumblr.com/', client.get_channel('232218346999775232'), client.get_channel('243542820189765633'), 30))
	loop.create_task(background_check_feed(loop, 'http://mindareadsoots.tumblr.com/', client.get_channel('290270624558088192'), client.get_channel('298828535894769665'), 90))
	loop.create_task(fear_of_death(180))
	
	print("Defining localization enumerations")
	global emotes
	emotes = dict()
	emotes.update({minda_server:':smolrubes:300822291229442048',lwu_server:':smolrubes:243554386549276672'})
	
	global blushemote
	blushemote = dict()
	blushemote.update({minda_server:':smolrubes:300822291229442048',lwu_server:':thatsmywife:244688656911171585'})
	
	global modrole
	modrole = dict()
	modrole.update({minda_server:  discord.utils.get(minda_server.roles, id='298390405169283072'),lwu_server : discord.utils.get(lwu_server.roles, id='282703165269213184')})
...
def on_message(message):
	...
	if message.server != None: #Generic Server
		print("[" + message.channel.server.name + "]\t" + "[" + message.channel.name + "] " + message.author.name + ": " + message.clean_content)
	...
	if message.server == None: #Private Message
		...
		if message.author == gio:
			...
			# elif message.content.startswith('!updebug'):
				...
				for s in lwu_server.emojis:
					print(s)
				for s in minda_server.emojis:
					print(s)
					#yield from client.send_message(workingChan, "<:smolrubes:243554386549276672>")
